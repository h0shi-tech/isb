...
def generate_keys(symmetric_key_path, public_key_path, private_key_path):
    ...
    Args:
        symmetric_key_path (str): Путь для сохранения зашифрованного симметричного ключа
...
def encrypt_file(input_file, private_key_path, symmetric_key_path, output_file):
    ...
    Args:
        input_file (str): Путь к входному файлу для шифрования
        private_key_path (str): Путь к файлу с закрытым ключом
        symmetric_key_path (str): Путь к файлу с зашифрованным симметричным ключом
